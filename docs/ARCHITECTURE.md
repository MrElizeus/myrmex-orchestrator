...
The structured planner boundary is: authoritative normalized-backlog snapshot -> deterministic planning request/context and fixed prompt -> external planner -> strict result validation -> immutable response -> immutable `proposed` plan revision. It is planning-only; it does not create work units, mutate DAGs, activate plans, or perform repository effects.
