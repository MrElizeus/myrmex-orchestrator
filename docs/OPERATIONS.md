...
`myrmex_plan_critic.prepare_critic_task` requires the persisted planner task, result, and exact proposed record, then persists a different critic task identity before transport. Its deterministic preflight establishes minimum coverage, scope, verification, dependency, risk, assumption, and human-gate findings that critic output cannot contradict. Reviews use exact `PASS`, `REVISE`, `BLOCKED`, or `INVALID` verdicts with digest-bound defect references. A PASS appends only a `reviewed` lifecycle record. The review is durable before that lifecycle append and before the critic receipt, so recovery finishes from accepted bytes without repeating transport. Activation remains exclusively owned by P1-012.

`myrmex-campaign plan-compile-preview <campaign> --plan-revision-id <plan_...> --expect-revision <n>` returns deterministic work orders, campaign WU specs, edges, traceability, and a compilation digest without changing campaign, event, artifact, or projection state. It requires a current reviewed head and one matching PASS critic receipt. `plan-compile-apply` accepts the same identity and required campaign revision, applies only non-conflicting WUs under CAS, and returns `APPLIED` or byte-preserving `REUSED`. Legacy `wu-add` remains supported and produces WUs without the optional work-order extension.

`myrmex-campaign dag <campaign> --plan-revision-id <plan_...> --expect-revision <n>` performs read-only semantic validation and returns `myrmex.dag-validation-result/v1`. Save that PASS receipt, then invoke `plan-activate` with its JSON file plus the exact reviewed record/review digest, campaign revision, activation-authority JSON, required human-decision JSON, request ID, and activation timestamp. Activation persists intent and PASS preconditions before appending deterministic `validated` and `active` lifecycle records; it then records the active projection and `myrmex.plan-activation/v1` receipt. Exact replay returns the same receipt without rewriting projection bytes. Generic plan-store APIs, planner, and critic remain unable to create a new active record, and activation never grants repository writes, commit, push, merge, release, or deployment.

`myrmex-campaign replan-trigger-record <campaign> --trigger-type <type> --summary <text> --evidence-json <file> --detected-at <RFC3339> --source-producer <name> --source-event-id <id> --expect-revision <n>` verifies every supplied evidence reference against its immutable campaign artifact and the current active-plan receipt before writing one `myrmex.replan-trigger/v1`. Evidence older than the activation campaign revision, unavailable artifacts, digest mismatches, and wrong-campaign references fail closed. Reordered evidence yields the same trigger ID; exact replay is read-only. `replan-trigger-list` reconstructs and revalidates the ledger. Neither command replans or mutates the active plan.
