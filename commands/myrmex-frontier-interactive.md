---
description: Run Myrmex frontier delegation and stop for plan review before implementation
agent: myrmex-orchestrator
---

Use the `myrmex-frontier-delegation` skill in **interactive** mode for this objective:

```text
$ARGUMENTS
```

Create repository-grounded context, obtain one structured frontier plan through `myrmex-frontier`, and show the plan before implementation. Do not push.
